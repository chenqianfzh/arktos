...
get get_ip_addr ${RUN_PREFIX}-kubemark-rp-master
...
for minion in $(gcloud compute instance-groups list-instances ${RUN_PREFIX}-minion-group --zone "${GCE_REGION}" --project "${GCE_PROJECT}")
do 
  echo "------------ $minion"
  get_ip_addr $minion
done
